...
kubectl create secret generic cart-connection -n devstore --from-literal=CUSTOMCONNSTR_DefaultConnection="Server=postgresql.infra.svc.cluster.local;Port=5432;Database=DBCart;User Id=postgres;Password=postgres;" -o yaml --dry-run=client > ./api-cart/cart-connection-secret.yaml
...
helm install --debug api-cart -f api-common.yaml -f ./api-cart/values.yaml ./api-template -n devstore
