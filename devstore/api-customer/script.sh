...
kubectl create secret generic customer-connection -n devstore --from-literal=CUSTOMCONNSTR_DefaultConnection="Server=postgresql.infra.svc.cluster.local;Port=5432;Database=DBCustomer;User Id=postgres;Password=postgres;" -o yaml --dry-run=client > ./api-customer/customer-connection-secret.yaml
...
helm install --debug api-customer -f api-common.yaml -f ./api-customer/values.yaml ./api-template -n devstore
