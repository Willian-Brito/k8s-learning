...
Se o Kubernetes fosse um sistema operacional:

- Pods / Deployments = processos
- YAMLs = binários
- Helm = apt / yum / brew

👉 Ele permite **instalar, atualizar, versionar e remover aplicações** no Kubernetes de forma padronizada.

### 📦 Conceitos principais

#### 1️⃣ Chart

Um Chart é um pacote Helm.
...
├── templates/        # manifests Kubernetes (Deployment, Service, etc)
└── charts/           # dependências (outros charts)
```

#### 2️⃣ Values

O `values.yaml` é onde você customiza o comportamento do chart.
...
ingress:
  enabled: true
```
👉 O mesmo chart pode subir **dev, hml, prod** mudando só o `values.yaml`

#### 3️⃣ Templates

Helm usa **Go Templates** para gerar YAMLs dinâmicos.
...
replicaCount: 3
```
👉 O Helm gera YAML Kubernetes válido.

#### 4️⃣ Release

Um **Release** é uma instalação de um chart em um namespace.

```bash
helm install api-identity ./api-template -n devstore
```
- Chart → modelo
- Release → instância rodando

📌 Você pode ter vários releases do mesmo chart.
...
**Atualizar**
```bash
helm upgrade api-identity ./api-template -n devstore
```

**Instalar ou atualizar (mais usado)**
```bash
helm upgrade --install api-identity ./api-template -n devstore
```
...
Se um deploy deu ruim:
```bash
helm history api-identity -n devstore
helm rollback api-identity 1 -n devstore
```
👉 Volta para uma versão anterior **em segundos**.

### 🧩 Boas práticas

- Sempre use `helm upgrade --install`
- Não hardcode valores (use `values.yaml`)
- Feature flags (`ingress.enabled`, `hpa.enabled`)
- Nunca commitar secrets no `values.yaml`
- Use `helm lint` e `helm template`
...
| --------------------- | ----------------- |
| Templates             | Overlays          |
| Lógica condicional    | Sem lógica        |
| Pacotes reutilizáveis | Customização      |
| Charts versionados    | Sem versionamento |

👉 Helm = empacotar

👉 Kustomize = customizar

### 🎯 Quando usar Helm?

**Use Helm quando:**
- Você tem múltiplos ambientes
- Precisa de rollback
- Quer padronizar deploys
- Mantém vários serviços
